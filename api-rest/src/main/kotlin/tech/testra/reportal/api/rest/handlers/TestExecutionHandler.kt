...
class TestExecutionHandler(
    ...
    fun findAll(req: ServerRequest): Mono<ServerResponse> =
        _testExecutionService.getExecutionsByProjectId(req.getProjectIdFromPath())
            .onErrorResume { throw it }
            .collectList()
            .flatMap { ok().contentType(APPLICATION_JSON_UTF8).body(fromObject(it)) }

    fun findById(req: ServerRequest): Mono<ServerResponse> =
        _testExecutionService.getExecutionById(req.getProjectIdFromPath(), req.getExecIdFromPath())
            .flatMap { ok().contentType(APPLICATION_JSON_UTF8).body(fromObject(it)) }
            .onErrorResume { throw it }

    fun createExecution(req: ServerRequest): Mono<ServerResponse> =
        _testExecutionService.createExecution(req.getProjectIdFromPath(), req.bodyToMono(TestExecutionModel::class.java))
            .flatMap { ok().contentType(APPLICATION_JSON_UTF8).body(fromObject(it)) }
            .onErrorResume { throw it }
