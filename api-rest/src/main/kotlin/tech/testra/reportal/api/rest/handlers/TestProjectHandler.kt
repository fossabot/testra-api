...
class TestProjectHandler(val _testProjectService: ITestProjectService) {
    ...
    fun getProjectById(req: ServerRequest): Mono<ServerResponse> =
        _testProjectService.getProjectById(req.getProjectIdFromPath())
    ...
    fun updateProject(req: ServerRequest): Mono<ServerResponse> =
        _testProjectService.updateProject(req.getProjectIdFromPath(), req.bodyToMono(ProjectModel::class.java))
            .flatMap { ok().contentType(APPLICATION_JSON_UTF8).body(fromObject(it)) }

    fun deleteProject(req: ServerRequest): Mono<ServerResponse> {
        return _testProjectService.deleteProjectById(req.getProjectIdFromPath())
            .flatMap {
                if (it) ok().build()
                else throw ProjectNotFoundException(req.getProjectIdFromPath())
            }
    }
}