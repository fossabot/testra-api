...
class TestGroupService(val _testGroupRepository: ITestGroupRepository) : ITestGroupService {
    ...
    override fun getOrAddGroup(groupName: String, projectId: String): Mono<String> {
        return _testGroupRepository.findByNameAndProjectId(groupName, projectId)
            .map { it.id }
            .switchIfEmpty(
                _testGroupRepository
                    .save(TestGroup(projectId = projectId, name = groupName, type = GroupType.FEATURE).toMono())
                    .map { it.id }
